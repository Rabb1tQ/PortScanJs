 ...
 */
 ...
 class Scanner {
  constructor(opt, argv) {
    let ports=[];
    let arrPort=argv.ports.split(",");
    for(let i=0;i<arrPort.length;i++){
      var  strPort = arrPort[i];
      if (strPort.indexOf("-")!=-1)
      {
          let temp01 = strPort.split('-');
          let min = temp01[0];
          let max =temp01[1];
          for (let j = min; j <= max; j++)
          {
               /*let strTmpPort = j.ToString();*/
              if (ports.indexOf(j)==-1)
              {
                ports.push(j);
              }
          }
      }
      else if (ports.indexOf(strPort)==-1)
      {
        ports.push(strPort);
      }
    }
    argv.ports=ports.join(",");
